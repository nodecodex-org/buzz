...
pub(super) fn add_verified_team(
    ...
    // member copy whose logical undo is a field revert.
...
/// `projection_hash`. The hash makes the hint exact-match gated: a hostile
/// `builtin_slug` with unrelated fields, a retired slug, and a slug whose
/// local definition has changed all fail and fall through to an ordinary copy
/// built from the embedded (authoritative) fields.
