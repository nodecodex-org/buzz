...
/// NOT the deleted record — so the boot orphan sweep
/// ([`crate::event_sync::reconcile_deleted_heads`]), which only has the head,
/// re-enqueues BOTH effects on restart retry.
pub(crate) fn tombstone_managed_agent_at(
    ...
    let result = (|| -> Result<(), String> {
        ...
        // purged — the same source the boot orphan sweep will use, so both
        // paths enqueue an identical archive.
