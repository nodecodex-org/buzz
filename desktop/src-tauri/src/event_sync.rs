...
/// Read `managed-agents.json` strictly for the deletion sweep: absent is an
/// empty store, but a malformed file is preserved as `.invalid` and fails loud
/// (mirrors [`crate::managed_agents::reconcile`]'s contract) — a truncated file
/// must never read as empty and tombstone live agents.
...
/// ([`migrate_personas_to_events`]/[`migrate_teams_to_events`]/
/// [`crate::managed_agents::reconcile`]): those retain a head for every live
/// disk record; this retracts a head that has NO live disk record.
///
...
///
/// Reads all three disk stores FIRST, before any tombstone: a malformed store
/// fails loud (and `managed-agents.json` is preserved as `.invalid`) so a
/// truncated file can never read as empty and orphan every head. Missing files
/// are legitimately empty — every record of that kind was deleted — so their
/// surviving heads are correctly tombstoned.
fn reconcile_deleted_heads_at(
    base_dir: &Path,
    keys: &nostr::Keys,
    db_path: &Path,
) -> Result<u32, String> {
    use crate::commands::{tombstone_managed_agent_at, tombstone_persona_at, tombstone_team_at};
    ...
    // Validate managed-agents.json first (it backs BOTH persona and agent
    // coordinates post-fold): a parse failure here aborts with an `.invalid`
    // backup before `read_persona_definitions` re-reads it.
    let agent_records = read_agents_strict(base_dir)?;
    ...
    // Keyed records are managed agents (keyless records are the personas
    // above); an empty pubkey has no 30177 coordinate.
    let agent_tags: HashSet<String> = agent_records
        .into_iter()
